...
use cosmwasm_std::{to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult, Uint128, WasmMsg, SubMsg, Reply, Addr};
...
pub fn reply(deps: DepsMut, _env: Env, msg: Reply) -> StdResult<Response> {
    match msg.id {
        ...
        id => Err(StdResult::generic_err(format!("Unknown reply id: {}", id))),
    }
}
    ...
    match msg.id {
        CW20_CONTRACT_REPLY_ID => {
            CONFIG.update(deps.storage, |mut config| -> StdResult<_> {
                config.money_cw20_contract.addr = Option::from(Addr::unchecked(res.contract_address));
                Ok(config)
            })
        }
        CW721_CONTRACT_REPLY_ID => {
            CONFIG.update(deps.storage, |mut config| -> StdResult<_> {
                config.spaceship_cw721_contract.addr = Option::from(Addr::unchecked(res.contract_address));
                Ok(config)
            })
        }
        _ => {}
