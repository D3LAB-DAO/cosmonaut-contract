...
fn main() {
    let execute_output_dir: &str = &args()
        .nth(1)
        .unwrap_or_else(|| DEFAULT_EXECUTE_RESULT_DIR.to_string());
    let query_output_dir: &str = &args()
        .nth(2)
        .unwrap_or_else(|| DEFAULT_QUERY_RESULT_DIR.to_string());
    ...
    query_all_cw721_msgs(&app, &cw721_contract_addr, ADDR1, ADDR2).write_to_file(query_output_dir);
}
