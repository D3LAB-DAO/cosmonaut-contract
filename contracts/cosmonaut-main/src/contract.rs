...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::AddFreightContract { address } => {
            execute::add_freight_contract(deps, address)
        }
        ...
        ExecuteMsg::SetMinter { minter } => {
            execute::set_minter_to_cw721_contract(deps, minter)
        }
        ...
        ExecuteMsg::BuyMoneyToken { amount } => {
            execute::buy_money_token(deps, info, amount)
        }
        ...
        ExecuteMsg::PlayGame { token_id, epoch } => {
            execute::play_game(deps, env, token_id, epoch)
        }
    }
}
